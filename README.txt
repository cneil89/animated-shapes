...
Working solution are included with the assignment so that you can see what a
correct solution looks like. These executables are located in the 'bin'
directory, adn are provided for 64 bit Windows (10 / 11).
...
- Only submit main.cpp and no ther files

YOU MUST INCLUDE THE FOLLOWING INFOR IN A COMMENT AT THE TOP OF main.cpp:
...
- If you did not get a specific feature to work (such as text centering)
  Please edxplain what you tried to do to get it to work, and partial marks
  may be given for effor.

---------------------------------------------------------------------------------------
Program Specification
---------------------------------------------------------------------------------------

In this assignment you will be writing a C++/SFML program with will read
descriptions of shapes from a configuration file, and the drawing those shapes
...
of a shape refers to its SFML default of the upp-left corner of the shape.
...
- Toggle whether or not the selected shap will be draawn
- Change the x an y celocity of the shay (-8 to 8)
...
UI Note: Do not worry if rapidly changing the scale parameter causes the
         ...
         Only be changed to remain within the rectangle. The wnewly scaled
...
taht the rest of the line specified. Lines in the config file can be one of the
following types, and lines can appear in any order in the file.

Window W H
- This line declares taht the SFML Window must be constructed 
